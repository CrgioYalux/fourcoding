...
const Editor: React.FC<EditorProps> = () => {
    ...
    return (
        ...
        <small>participants: {state.room?.participants.filter(p => p !== null).join(' - ')}</small>
        <div className='Editor'>
            <EditorTextarea editing='HTML' value={HTML} onChange={
                (event) => {
                    setHTML(event.target.value);
                    actions.sendHTMLEditor({ html: event.target.value });
                }
            } />
            <EditorTextarea editing='CSS' value={CSS} onChange={
                (event) => {
                    setCSS(event.target.value);
                    actions.sendCSSEditor({ css: event.target.value });
                }
            } />
            <EditorTextarea editing='JS' value={JS} onChange={
                (event) => {
